...
params:
  sample_info_file: "/hpcnfs/scratch/DIMA/gmariani/rna_seq/projects/Mapelli_DESeq_GO/data/samples_info.tsv"
  dds_file: "/hpcnfs/scratch/DIMA/gmariani/rna_seq/projects/Mapelli_DESeq_GO/data/deseq2.dds.RData"
  introduction_file: "/hpcnfs/scratch/DIMA/gmariani/rna_seq/projects/Mapelli_DESeq_GO/data/main_description.txt" 
---
...
text_lines <- readLines(params$introduction_file)
cat(text_lines, sep = "\n")
```
...
3) Read alignment with [STAR](https://github.com/alexdobin/STAR) STAR stands for Spliced Transcripts Alignment to a Reference, and has been shown to have high accuracy and outperforms other aligners by more than a factor of 50 in mapping speed, but it is memory intensive.
4) Estimation of transcript and gene expression with [Salmon](https://combine-lab.github.io/salmon/) Salmon is used to take STAR alignments to the transcriptome and fastly quantify them, with minimal computational resources.
...
The steps from 1 to 4 have been performed using [nf-core/rnaseq](https://nf-co.re/rnaseq) v3.14.0 which is a bioinformatics pipeline that can be used to analyse RNA sequencing data obtained from organisms with a reference genome and annotation. 
Data have been normalized with variance stabilizing transformation (VST) normalization using Deseq2. 
...
Below we present the datatable containing all of the samples analyzed in this report.  
...
Each sample is described by:
  ...
  - batch: the batch relative to each sample

  - condition: the condition as defined in the previous step of RNA seq analysis
...
Below are the comparison groups used in this analysis.

Each group consists of two subsets of samples: one experimental and one control.

Differential gene expression analysis is performed by comparing the experimental samples against the control samples within the same group.
...
## All-samples PCA Analysis

Below we present the PCA analysis conducted on the entire sample dataset.

Principal Component Analysis (PCA) was performed to explore the global variance in gene expression profiles across all samples. 

The main goal of this analysis is to assess whether samples cluster according to their experimental conditions, indicating biologically meaningful differences. 

PCA also helps to identify potential outliers and to detect the presence of batch effects or other sources of unwanted variation. 

By reducing the high-dimensional gene expression data into a few principal components, PCA provides a visual summary of the overall structure of the dataset, which is useful for validating sample quality and guiding downstream analysis decisions.
...
A sample-to-sample Spearman correlation heatmap provides a global view of the similarity between expression profiles across all samples. 
By using the Spearman correlation coefficient, which measures the rank-based association between samples, this analysis is robust to outliers and non-linear relationships. 

The heatmap is useful for assessing whether biological replicates cluster together and whether samples group according to experimental conditions. 

It also helps to identify potential outliers, mislabeled samples, or unwanted sources of variability. 
Below a short description on how to interpret the data.
...
### **Spearman Correlation Heatmap: General Interpretation**
...
#### 📉 **Lower Correlation Values** (e.g., < 0.90–0.80)  
🔹 *Between replicates of the same condition:* 
...
Below we present a barplot of the total read counts by sample.

Plotting the library sizes, defined as the total number of reads (or counts) per sample after preprocessing, is a fundamental quality control step in RNA-seq analysis. 
This visualization helps to assess the consistency of sequencing depth across samples, which is important to ensure reliable comparisons between conditions. 

Large differences in library sizes may indicate technical biases or issues during library preparation or sequencing, and can affect downstream normalization and differential expression analysis. 
Identifying such imbalances early allows for appropriate normalization strategies and increases the robustness of the results.
...
A violin plot of VST-normalized counts provides an overview of the global distribution of gene expression values across samples after normalization. 
Variance stabilizing transformation (VST) reduces the dependence of the variance on the mean, making the data more homoscedastic and suitable for visualization and clustering. 

This plot allows for the detection of potential outliers, technical biases, or inconsistencies in distribution across samples, which could affect downstream analyses. 

A consistent distribution of VST counts across samples suggests successful normalization and comparable expression profiles.
