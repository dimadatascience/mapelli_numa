---
title: "Single Comparison Report"
output:
...
The first steps to start the analysis in R is to load the input data and establish the thresholds for the analysis:

- min_sample = ", min_sample, ", minimum number of samples where the gene needs to have at least 1 read;
- logfc = ", logfc, ", which represents the ratio between the expression level of a gene in the conditions considered, e.g. ", params$exp$exp_name, "and", params$contr$contr_name, ", expressed in  logarithmic scale (base 2); a positive log fold change for a gene, greater than 1, means that the expression of that specific gene is increased in ", params$exp$exp_name, " with respect to", params$contr$contr_name, "by a multiplicative factor 2^logfc;
- qvalue = ", qvalue, ", that can be interpreted as false positive rate, the proportion of false positives among all positive results, which means avoid to detect differential expression of a gene that is not differentially expressed.
...
for (grp in levels(sample_info$sample_group)) {
  ...
  if (length(exp_samples) > 0 && length(ctrl_samples) > 0) {
    ...
    warning(paste("Il gruppo", grp, "non ha un confronto valido (exp + ctrl)"))
  }
}
...
The main goal of this analysis is the same as the PCA conducted on all samples: 
to assess whether samples cluster according to their experimental conditions, indicating biologically meaningful differences. 
...
The MA plot is a widely used visualization in differential expression analysis that displays the relationship between the average expression (A) and the log fold change (M) for each gene. 
It helps to identify genes that are significantly up- or down-regulated between conditions. 
The x-axis represents the mean expression level across samples, while the y-axis shows the log fold change between groups. 
Genes with large fold changes and sufficient expression tend to appear farther from the center horizontal line (log fold change = 0). 
This plot allows for quick assessment of the overall distribution of differential expression results, highlights systematic biases, and aids in detecting artifacts or trends related to expression intensity.
...
Genes with both large fold changes and high statistical significance appear in the upper left (downregulated) and upper right (upregulated) corners of the plot. 
This visualization effectively highlights genes that are both biologically relevant and statistically significant, facilitating the identification of potential targets for further study.
...
In this table we can find all the genes identified in the previous RNA seq analysis.
...
if (nrow(gseaA_list[["BP"]]@result) > 0) {
  ...
  cat("### GO - Biological Processes -", titleA, ": No enrichment found, P value cutoff:", pvalueCutoff, "\n\n")
}
```
...
if (nrow(gseaA_list[["CC"]]@result) > 0) {
  ...
  cat("### GO - Cellular Components -", titleA, ": No enrichment found, P value cutoff:", pvalueCutoff, "\n\n")
}
```
...
if (nrow(gseaA_list[["MF"]]@result) > 0) {
  ...
  cat("### GO - Molecular Functions -", titleA, ": No enrichment found, P value cutoff:", pvalueCutoff, "\n\n")
}
```
...
```{r over_representation_analysis_A, fig.width=10, results="asis"}
