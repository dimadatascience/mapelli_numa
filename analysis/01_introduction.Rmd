...
params:
  ...
  logfc: log2(2)
  qvalue: 0.05
  descriptions_file: "./data/descriptions_01.yaml" 
---
...
# 1. Get number of batch
n_batches <- length(unique(sample_info$batch))
batch_effect <- n_batches > 1

# identify technical controls
technical_controls <- subset(sample_info, sample_type %in% c("technical_control"))
```
...
__Interpretation PCA Analysis__

```{r PCA_interpretation, results="asis", echo=FALSE}
cat(descriptions$PCA, sep = "\n")
```
...
```{r correlation_heatmap_interpretation, results="asis", echo=FALSE}
cat(descriptions$correlation_heatmap, sep = "\n")
```
...
```{r library_sizes_interpretation, results="asis", echo=FALSE}
cat(descriptions$library_sizes, sep = "\n")
```
...
```{r violin_plot_interpretation, results="asis", echo=FALSE}
cat(descriptions$violin_plot, sep = "\n")
```
...
- [Synchronized NuMA- vs Synchronized NuMA+](02_numa_vs_nonuma_s.html)<span class="label btn-warning">new report</span>

- [Non-synchronized NuMA- vs Non-synchronized NuMA+](03_numa_vs_nonuma_ns.html)<span class="label btn-warning">new report</span
